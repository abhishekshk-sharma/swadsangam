...
<div class="d-flex justify-between align-center mb-4">
    <h2 style="font-size:1.2rem;font-weight:600;color:var(--gray-800);">Managers</h2>
...
<div class="content-card">
    <div class="card-header">
        ...
        <span id="total-count" style="font-size:0.8rem;color:var(--gray-500);">{{ $managers->total() }} total</span>
    </div>
    <div class="table-responsive">
        <table class="table">
            <thead>
                <tr>
                    <th>#</th><th>Name</th><th>Email</th><th>Phone</th><th>Branch</th><th>Assigned Staff</th><th>Status</th><th>Actions</th>
                </tr>
            </thead>
            <tbody id="table-body">
                @include('admin.managers._tbody')
            </tbody>
        </table>
    </div>
    <div id="pagination">
        @include('admin.managers._pagination')
    </div>
</div>
...
<script>
const ajaxUrl = '{{ route('admin.managers.ajax') }}';
let currentPage = 1, debounceTimer;

function getParams(page) {
    const v = id => document.getElementById(id)?.value ?? '';
    const params = new URLSearchParams({ page });
    ['f-search','f-phone','f-status'].forEach(id => {
...
['f-status','f-per-page'].forEach(id => {
