...
<div style="margin-bottom:24px;">
    <h1 style="font-size:1.2rem;font-weight:700;color:#111827;margin:0;"><i class="fas fa-layer-group" style="margin-right:8px;color:#d97706;"></i>Table Categories</h1>
    <p style="font-size:13px;color:#6b7280;margin:4px 0 0;">Global table category templates</p>
</div>
...
<div style="display:grid;grid-template-columns:1fr 360px;gap:20px;align-items:start;">
            ...
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr id="row-{{ $cat->id }}" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                        ...
                        <td style="{{ $td }}">
                            <span style="background:#f3f4f6;color:#374151;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;">{{ $cat->tables_count }}</span>
                        </td>
                        <td style="{{ $td }}">
                            <div style="display:flex;gap:6px;">
                                ...
                                <form action="{{ route('superadmin.table-categories.destroy', $cat->id) }}" method="POST" style="margin:0;" onsubmit="return confirm('Delete {{ $cat->name }}?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr id="editrow-{{ $cat->id }}" style="display:none;background:#fffbeb;">
                        <td colspan="3" style="padding:12px 16px;">
                            <form action="{{ route('superadmin.table-categories.update', $cat->id) }}" method="POST" style="display:flex;gap:10px;align-items:center;">
                                @csrf @method('PUT')
    ...
    <div class="content-card">
        ...
        <div style="padding:20px;">
            <form action="{{ route('superadmin.table-categories.store') }}" method="POST">
                @csrf
                <div style="margin-bottom:16px;">
                    <label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;">Category Name *</label>
                    <input type="text" name="name" required placeholder="e.g. VIP, Regular, Outdoor"
