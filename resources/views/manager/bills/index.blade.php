...
<div class="content-card mb-4">
    <div class="card-header">
        <div class="card-title"><i class="fas fa-filter me-2"></i>Filter Bills</div>
    </div>
    <div class="card-body">
        <form method="GET" action="{{ route('manager.bills.index') }}" id="billFilterForm">
            <div style="display:flex;gap:14px;align-items:flex-end;flex-wrap:wrap;">
                ...
                <div style="display:flex;gap:8px;align-items:flex-end;">
                    ...
                    <a href="{{ route('manager.bills.index') }}" class="btn btn-secondary">Clear</a>
                </div>
            </div>

            <div style="margin-top:16px;padding-top:14px;border-top:1px solid #e5e7eb;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
...
<div style="display:grid;grid-template-columns:repeat(2,1fr) repeat(3,1fr);gap:16px;margin-bottom:20px;">
    <div style="background:#fff;border-radius:12px;padding:20px;border:1px solid #e5e7eb;border-left:4px solid #4facfe;text-align:center;">
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:8px;">Total Bills</div>
        <div style="font-size:28px;font-weight:700;color:#111827;">{{ $totalBills }}</div>
        <div style="font-size:12px;color:#9ca3af;margin-top:4px;">{{ $filterLabel }}</div>
    </div>
    <div style="background:#fff;border-radius:12px;padding:20px;border:1px solid #e5e7eb;border-left:4px solid #43e97b;text-align:center;">
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:8px;">Total Revenue</div>
        <div style="font-size:28px;font-weight:700;color:#111827;">₹{{ number_format($totalAmount, 2) }}</div>
    </div>
    <div style="background:#fff;border-radius:12px;padding:16px 20px;border:1px solid #e5e7eb;border-left:4px solid #16a34a;text-align:center;">
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:6px;">💵 Cash</div>
        <div style="font-size:20px;font-weight:700;color:#15803d;">₹{{ number_format($paymentTotals['cash'], 2) }}</div>
    </div>
    <div style="background:#fff;border-radius:12px;padding:16px 20px;border:1px solid #e5e7eb;border-left:4px solid #7c3aed;text-align:center;">
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:6px;">📱 UPI</div>
        <div style="font-size:20px;font-weight:700;color:#6d28d9;">₹{{ number_format($paymentTotals['upi'], 2) }}</div>
    </div>
    <div style="background:#fff;border-radius:12px;padding:16px 20px;border:1px solid #e5e7eb;border-left:4px solid #0284c7;text-align:center;">
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:6px;">💳 Card</div>
        <div style="font-size:20px;font-weight:700;color:#0369a1;">₹{{ number_format($paymentTotals['card'], 2) }}</div>
    </div>
</div>
...
<div class="content-card mb-4">
    <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        <div class="card-title"><i class="fas fa-chart-bar me-2" style="color:var(--blue-500);"></i>Bills & Revenue Chart</div>
        <div style="display:flex;gap:8px;">
            <button onclick="switchChart('bar')" id="chartBtnBar" style="padding:5px 14px;border-radius:6px;border:1px solid var(--blue-500);background:#eff6ff;color:var(--blue-600);font-size:12px;font-weight:600;cursor:pointer;">Bar</button>
    ...
    <div class="card-body"><canvas id="billsChart" style="max-height:280px;"></canvas></div>
</div>
@endif

{{-- Bulk Delete --}}
<div class="content-card mb-4" style="border-left:4px solid #dc2626;">
    <div class="card-header"><div class="card-title" style="color:#dc2626;"><i class="fas fa-trash-alt me-2"></i>Bulk Delete / Restore Bills</div></div>
    <div class="card-body">
        <form method="POST" action="{{ route('manager.bills.bulkHide') }}" onsubmit="return confirm('Are you sure?')">
            @csrf @method('PATCH')
            <div style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;">
                ...
                <div id="bulk-day" style="display:none;flex-direction:column;gap:6px;"><label class="form-label">Day</label><input type="date" name="day" class="form-control"></div>
                <div id="bulk-month" style="display:none;flex-direction:column;gap:6px;"><label class="form-label">Month</label><input type="month" name="month" class="form-control"></div>
                <div id="bulk-year" style="display:none;flex-direction:column;gap:6px;"><label class="form-label">Year</label><select name="year" class="form-select">@for($y=now()->year;$y>=2020;$y--)<option value="{{ $y }}">{{ $y }}</option>@endfor</select></div>
                <div id="bulk-from" style="display:none;flex-direction:column;gap:6px;"><label class="form-label">From</label><input type="date" name="date_from" class="form-control"></div>
                <div id="bulk-to" style="display:none;flex-direction:column;gap:6px;"><label class="form-label">To</label><input type="date" name="date_to" class="form-control"></div>
                <div style="display:flex;gap:8px;">
...
<div class="content-card">
    <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        <div class="card-title"><i class="fas fa-list me-2"></i>Bills List
            <span style="font-size:12px;font-weight:400;color:var(--gray-500);">— {{ $filterLabel }}</span>
        ...
        <table class="table mb-0">
            <thead>
                <tr style="background:#f9fafb;">
                    <th>Bill #</th><th>Table</th><th>Amount</th><th>Payment</th><th>Paid At</th><th>Status</th><th style="text-align:center;">Actions</th>
                </tr>
            </thead>
            <tbody>
                @forelse($bills as $bill)
                <tr style="{{ $bill->bill_hidden ? 'opacity:.55;' : '' }}">
                    <td style="font-weight:700;">#{{ $bill->id }}</td>
                    <td>{{ $bill->is_parcel ? '📦 Parcel' : 'T'.($bill->table?->table_number ?? '?') }}</td>
                    <td style="font-weight:700;color:#15803d;">₹{{ number_format($bill->grand_total ?? $bill->total_amount, 2) }}</td>
                    <td>
                        @php $pm = $bill->payment_mode; @endphp
                        <span class="badge {{ $pm==='cash'?'badge-success':($pm==='upi'?'badge-info':'badge-neutral') }}">
                            {{ $pm==='cash'?'💵':($pm==='upi'?'📱':'💳') }} {{ ucfirst($pm ?? '—') }}
                        </span>
                    </td>
                    <td style="font-size:13px;color:var(--gray-500);white-space:nowrap;">{{ $bill->paid_at?->format('d M Y, h:i A') ?? '—' }}</td>
                    <td>
                        @if($bill->bill_hidden)
                            <span class="badge" style="background:#fee2e2;color:#dc2626;"><i class="fas fa-trash-alt me-1"></i>Deleted</span>
                        @else
                    ...
                    <td style="text-align:center;">
                        <div style="display:flex;gap:6px;justify-content:center;">
                            ...
                            <form action="{{ route('manager.bills.toggleHidden', $bill->id) }}" method="POST" style="margin:0;">
                                @csrf @method('PATCH')
                                <button type="submit" class="btn btn-sm" style="background:{{ $bill->bill_hidden ? '#16a34a' : '#dc2626' }};color:#fff;border:none;">
                                    <i class="fas fa-{{ $bill->bill_hidden ? 'undo' : 'trash-alt' }} me-1"></i>{{ $bill->bill_hidden ? 'Restore' : 'Delete' }}
                                </button>
                            </form>
                        </div>
                    </td>
...
function setBillMode(mode) {
    ...
    ['all','cash','upi','card'].forEach(function(m){ var btn=document.getElementById('bmode-'+m); if(!btn) return; var a=m===mode; btn.style.borderColor=a?'#2563eb':'#d1d5db'; btn.style.background=a?'#eff6ff':'#fff'; btn.style.color=a?'#2563eb':'#6b7280'; });
...
function buildChart(type) {
    ...
    billChart = new Chart(document.getElementById('billsChart'), {
        ...
        data: {
            ...
            datasets: [
                { label: 'Bills Count', data: @json($chartData->pluck('count')), backgroundColor: 'rgba(59,130,246,0.7)', borderColor: '#2563eb', borderWidth: 2, yAxisID: 'y', tension: 0.4 },
                { label: 'Revenue (₹)', data: @json($chartData->pluck('revenue')->map(fn($v) => round($v, 2))), backgroundColor: 'rgba(22,163,74,0.15)', borderColor: '#16a34a', borderWidth: 2, yAxisID: 'y1', tension: 0.4, fill: type==='line' }
            ]
        },
        options: { responsive: true, interaction: { mode: 'index', intersect: false }, plugins: { legend: { position: 'top' } },
            scales: { y: { type:'linear', position:'left', title:{display:true,text:'Bills'} }, y1: { type:'linear', position:'right', title:{display:true,text:'Revenue (₹)'}, grid:{drawOnChartArea:false} } } }
    });
}
function switchChart(type) {
    ['Bar','Line'].forEach(function(t){ var btn=document.getElementById('chartBtn'+t); btn.style.background=t.toLowerCase()===type?'#eff6ff':'#fff'; btn.style.borderColor=t.toLowerCase()===type?'var(--blue-500)':'#d1d5db'; btn.style.color=t.toLowerCase()===type?'var(--blue-600)':'#6b7280'; });
    buildChart(type);
}
