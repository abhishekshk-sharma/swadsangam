 ...
 *
 * Each layout must define window.ORDER_POLL before including this script:
 *   window.ORDER_POLL = { panel: 'waiter'|'cook'|'cashier'|'cashier_parcels'|'admin' };
 */
(function () {
    'use strict';
    ...
    // ── Busy guard — don't reload while user is editing ───────────────────────
    function isBusy() {
    ...
    function applyBadge(el, status) {
        ...
        var cls = STATUS_CLS[status] || '';
        cls.split(' ').forEach(function (x) { if (x) el.classList.add(x); });
    ...
    function orderMsg(order, newStatus) {
        ...
        var panelMsgs = msgs[panel] || {};
        return panelMsgs[newStatus] || ('Order #' + id + ' → ' + newStatus);
    }
    ...
    function updateCard(order) {
        ...
        // Waiter: show/hide serve button and checkout section
        if (panel === 'waiter') {
        ...
        order.items.forEach(function (item) {
            ...
            // Hide any open edit forms for this item
            var editForm = document.getElementById('cedit-' + item.id) ||
    ...
    function buildCashierCard(order) {
        ...
        div.innerHTML =
            ...
            '</div><span class="px-3 py-1 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-800" data-order-status-badge>' + order.status.charAt(0).toUpperCase() + order.status.slice(1) + '</span></div>' +
            '<div class="space-y-2 mb-4">' + itemsHtml + '</div>' +
            notesHtml +
            '<div class="pt-3 border-t">' +
            '<div class="font-bold text-xl text-green-600 mb-4" data-order-total>Total: ₹' + total + '</div>' +
    ...
    // ── Pending count (cashier payments header) ───────────────────────────────
        ...
        .then(function (data) {
            ...
            data.orders.forEach(function (order) {
                ...
                if (s.status !== order.status) {
                    ...
                    // Remove card when terminal status on cashier/waiter
                    if (['paid', 'cancelled'].includes(order.status) && (panel === 'cashier' || panel === 'waiter')) {
                        var card2 = document.querySelector('[data-order-id="' + order.id + '"]');
                        if (card2) {
                            card2.style.transition = 'opacity .3s,transform .3s';
                            card2.style.opacity = '0';
                            card2.style.transform = 'translateY(-6px)';
                            setTimeout(function () {
                                card2.remove();
                                updatePendingCount();
                    ...
                    // cashier_parcels: remove when paid/cancelled
                    if (['paid', 'cancelled'].includes(order.status) && panel === 'cashier_parcels') {
                        var card3 = document.querySelector('[data-order-id="' + order.id + '"]');
                        if (card3) {
                            card3.style.transition = 'opacity .3s,transform .3s';
                            card3.style.opacity = '0';
                            card3.style.transform = 'translateY(-6px)';
                            setTimeout(function () { card3.remove(); }, 350);
                        }
                        return;
                    }

                    // Cook/admin: reload for status changes needing full re-render
                    if ((panel === 'cook' || panel === 'admin') && ['pending', 'preparing', 'ready', 'cancelled'].includes(order.status)) {
                        safeReload(1500);
                    }
                }
                ...
                order.items.forEach(function (item) {
                    ...
                    if (prev !== item.status) {
                        s.items[iid] = item.status;
                        if (item.status === 'prepared')  toast('✓ "' + item.name + '" in Order #' + order.id + ' is prepared.', 'success');
                        if (item.status === 'cancelled') toast('❌ "' + item.name + '" in Order #' + order.id + ' was cancelled.', 'danger');
                        updateCard(order);
                    }
                });
            });
